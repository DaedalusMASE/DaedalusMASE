...
def plot_orbit_kp_scatter(OrbitFilename, kp_startYear, from_date_str, to_date_str, from_Altitude, to_Altitude):
    ...
    Args:
        OrbitFileName: the filename of the csv file describing the satellite orbit. Can be relative path or full path. If not found, the function tries to find the file in Orbit_Files_Path which can br set by set_OrbitFilesPath function.
    ...
    with open( orbit_filename_to_load ) as CSVfile:        
        ...
        for row in CSVreader: # for each satellite position
            ...
            if CURR_time == None:
                print( "ERROR during Kp-scatter while reading", orbit_filename_to_load, ": Wrong time format:", row[Time_idx] )
                return 0, 0, "", 0 # <<<<
    ...
    fig.update_layout( width=3000, height=2000, showlegend=False, coloraxis_showscale=False, title="Orbit file:" + orbit_filename_to_load + "<br>" + "Kp values start year is " + str(kp_startYear) + "<br>" + "<b>Kp Indices during " + from_date_str + " - " + to_date_str +  " for Altitudes: "+str(from_Altitude)+"-"+str(to_Altitude)+" km</b>")
...
def plot_orbit_heatmap( OrbitFilename ):
    ...
    Args:
        OrbitFileName: the filename of the csv file describing the satellite's orbit. Can be relative path or full path. If not found, the function tries to find the file in <Orbit_Files_Path> which can br set by <set_OrbitFilesPath> function.
    """
    ########
    if path.exists( OrbitFilename ):
        orbit_filename_to_load = OrbitFilename

"""
sub_heating_sources.plot_polar_north

**Description**:
_____________________________________________________________________________________________________________________

Plot quantities in 2D northern polar plot
_____________________________________________________________________________________________________________________
_____________________________________________________________________________________________________________________
...
def plot_polar_north(param, Bound_lat, orbit_file,timer,savefig=True):
...
#         cbJH.set_label('Joule Heating Rate [$mW/m^{3}$]')
    ...
    if param == 'σO+n':
        ...
        fig.text(0.35, 0.13, "%s" % timqe_plot.strftime("%d %b %Y"), fontsize=15)
        fig.text(0.68, 0.13, "%s" % time_plot.strftime("%H:%M:%S"), fontsize=15)
...
#         cbJH.set_label('Pendersen Conductivity ($10^{-5}$ mho/m)')
        fig.text(0.30, 0.90, 'Pendersen Conductivity', fontsize=17, fontweight='bold')
        fig.text(0.05, 0.13, "%s" % time_plot.strftime("%d %b %Y"), fontsize=15)
        fig.text(0.68, 0.13, "%s" % time_plot.strftime("%H:%M:%S"), fontsize=15)
        fig.text(0.68, 0.90, "[$10^{-5} mho/m$]" , fontsize=17)
...
#         cbJH.set_label('$N_{e}$ ($m^{-3}$)')
    ...
    if savefig:
        plt.savefig('Figures/'+param+'_polar_north.jpg',dpi=300)
    plt.show()
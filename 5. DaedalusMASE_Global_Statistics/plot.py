...
def plotVariable( RegionName, Variable_toPlot, Units_of_Variable="", Variable_toPlot_longname="" ):
    """
...
def plotVariable_KpSeparated( RegionName, Variable_toPlot, Units_of_Variable="", Variable_toPlot_longname="" ):
    """
        Creates the following plots based on the values of the chosen Variable:
            1. Variable vs Magnetic Latitude. The Bins' ranges and mean values are also displayed
            2. Variable vs Magnetic Local Time. The Bins' ranges and mean values are also displayed
...
def plotComparisonOfResults( results_filename1, results_filename2, Variable_toPlot, PlotTitle="" ):
    """
    ...
    RegionDescription = ""
    # load results no1
    LoadResults_CDF( results_filename1, Variable_toPlot )
    for B in Bins:
    ...
    for B in Bins:
        B.reset()
    # load results no2
    LoadResults_CDF( results_filename2, Variable_toPlot )
    for B in Bins:
...
def CalculateStandardDeviation( Data ):
    """
        Args: a list of real numbers
        Returns: the standard deviation of the numbers inside the argument
    """
...
def plotColorSpread_KpSeparated( RegionName, Variable_toPlot, Variable_toPlot_longname="" ):
    """
...
def plotPDFperSubBin( TIEGCMresults_filename, ORBITresults_filename, RegionName, Variable_toPlot, Variable_toPlot_longname="" ):
    """
    ...
    for input_file_idx in range(0, len(DataFiles)):
        ...
        LoadResults_CDF( aDataFile, Variable_toPlot, loadGlobalValues=False, loadTimeValues=False, loadMagLatValues=True, loadMLTvalues=True, loadAltValues=True, loadLatValues=False )
...
def executeStatTest( TIEGCMresults_filename, ORBITresults_filename, RegionName, VariableToLoad, number_of_samples=50 ):
    """
    ...
    LoadResults_CDF( TIEGCMresults_filename, VariableToLoad, loadGlobalValues=False, loadTimeValues=False, loadMagLatValues=False, loadMLTvalues=False, loadAltValues=False, loadLatValues=False )
    for B in Bins:
        if B.ID.startswith(RegionName):
    ...
    for B in Bins:
        B.reset()
    LoadResults_CDF( ORBITresults_filename, VariableToLoad, loadGlobalValues=False, loadTimeValues=False, loadMagLatValues=False, loadMLTvalues=False, loadAltValues=False, loadLatValues=False )
    for B in Bins:
        if B.ID.startswith(RegionName):

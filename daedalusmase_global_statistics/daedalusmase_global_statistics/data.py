...
def getBinByItsProperties( MLT_min, MLT_max, MagLat_min, MagLat_max, Altitude_min, Altitude_max, Kp_min, Kp_max ):
    """
        Returns:
            the bin object which has been defined by the same ranges as the arguments
    """
    CorrectBin = None
    for B in Bins:
...
def getBinByItsID( aBinID ):
    """
        Returns:
            the Bin object wich has the same ID as the argument
    """
    CorrectBin = None
    for B in Bins:
        if  B.ID == aBinID:
...
def CreateResults_CDF( ResultsFilename,  CALCULATIONS_Title="", CALCULATIONS_Description="", CALCULATIONS_RegionName="", CALCULATIONS_OrbitFilesPath="", CALCULATIONS_TIEGCMfolder=""):
    """
    Creates a results NetCDF file and its structure. The file will contain no data.  
    ...
    Args:
            ...
            CALCULATIONS_Title: useful information to be saved in the netcdf file.  
            CALCULATIONS_Description: useful information to be saved in the netcdf file.  
            CALCULATIONS_RegionName: useful information to be saved in the netcdf file.  
            CALCULATIONS_OrbitFilesPath: useful information to be saved in the netcdf file.  
            CALCULATIONS_TIEGCMfolder: useful information to be saved in the netcdf file.  
    """
...
def load_results( filepath, VariableToLoad, loadBinValues=True, loadGlobalValues=True, loadTimeValues=True, loadMagLatValues=True, loadMLTvalues=True, loadAltValues=True, loadLatValues=True, loadKpValues=True ):
    """
    Reads the calculation results from a netcdf result-file or a result-folder with many netcdf files and fills with data the corresponding Bins.
...
def calc_stats_for_orbit( TIEGCM_filesPath, Orbit_filesPath, ResultsFilename ): 
    """
    Reads the orbit positions and fills the correct Bin with values for each position.  
    It stores the result in a netCDF file.
...
def calc_stats_for_tiegcm( TIEGCMfilesPath, ResultFilesPath ):
    """
    Reads the TIEGCM files and fills the correct Bin with values for each position. It stores the result in a netCDF file.  
    ...
    It also tells the thread to store the result files into ResultFilesPath.  
    There will be a result-file for each TIEGCM-file. These can be loaded all together as a directory.
    
    Args:
        TIEGCM_filesPath: the folder which has all TIEGCM netcdf files. Needed to read the Kp index for each satellite position.
        ResultFilesPath: the netcdf file where the results of this calculation will be stored.
